...
try {
    ...
    $stmt = $conn->query("SELECT COUNT(*) FROM contracts WHERE is_active = 1 AND contract_end >= CAST(GETDATE() AS date)");
    ...
    $stmt = $conn->query("SELECT COUNT(*) FROM contracts WHERE is_active = 1 AND contract_end BETWEEN CAST(GETDATE() AS date) AND DATEADD(day, 90, CAST(GETDATE() AS date))");
