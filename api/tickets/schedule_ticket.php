...
try {
    ...
    if ($workStart === null) {
        // Clear the schedule
        $sql = "UPDATE tickets SET work_start_datetime = NULL, updated_datetime = GETDATE() WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$ticketId]);
    } else {
        // Set the schedule
        $sql = "UPDATE tickets SET work_start_datetime = ?, updated_datetime = GETDATE() WHERE id = ?";
        $stmt = $conn->prepare($sql);
